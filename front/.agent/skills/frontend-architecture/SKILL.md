...
description: Comprehensive architecture, coding standards, and rules for the FinTreX frontend (Angular 19, Clean Architecture).
---
...
| State | Angular Signals for UI State, RxJS for Async/HTTP (avoid unnecessary Subjects) |
...
-   L¦¦ guards/                    # Route guards (functional)
-
+¦¦ data/                          # Infrastructure layer — external communication
-   +¦¦ repositories/              # Concrete implementations of core/interfaces/
-   L¦¦ external-services/         # Third-party integrations (Stripe, market APIs)
-
L¦¦ presentation/                  # UI layer
    +¦¦ layout/                    # App shell (sidebar, navbar, router-outlet)
    -   L¦¦ main-layout/           # Role-based main layout
    ...
    -   L¦¦ pipes/                 # Common pipes (currency-format, ...)
    L¦¦ features/                  # Lazy-loaded feature modules
...
-   +¦¦ asset-search/
-   L¦¦ watchlist-table/
L¦¦ services/                    # Feature-specific services (optional)
```
---
...
- **Fallback is forbidden unless the specification explicitly requires it.**
- ? NEVER silently coerce invalid state into a default value (`role ?? 'USER'`, fake placeholder data, empty arrays used to mask backend failure, automatic redirect to an unrelated page, hidden `try/catch` recovery that changes behavior).
- ? NEVER add frontend fallback logic for missing roles, unsupported enum values, malformed API payloads, missing config, or broken route data.
- ? ALWAYS fail explicitly when a required value is missing or invalid: throw, return an error state, block rendering, or surface a clear validation/API error.
- ? ALWAYS treat contract mismatches as defects to be fixed at the source, not conditions to be hidden in the UI.
...
### 11.8 Testing Standards
```ts
// ❌ WRONG — relying on SCSS/HTML elements for tests
const el = fixture.debugElement.query(By.css('.button-submit'));

// ✅ ALWAYS — use data-testid
const el = fixture.debugElement.query(By.css('[data-testid="submit-button"]'));
```
- **Never select elements by CSS classes or tags** in unit or E2E tests. This makes tests brittle.
- **Always use data-testid** attributes for querying DOM elements.
...
### 14.0 Component Shell Standard

Every non-trivial page and feature component must follow the same structural pattern used by components such as `public-navbar` and the auth pages.

**HTML structure rule:**
- Use a single root block class per component, typically `feature-name-c`.
- Build the template as a wrapped shell, not as a flat list of unrelated siblings.
- The expected hierarchy is: root block -> wrapper -> container -> shell -> semantic sections/panels/content groups.
- Keep internal elements under the same block namespace with BEM-style names such as `feature-name-c__wrapper`, `feature-name-c__container`, `feature-name-c__panel`, `feature-name-c__section`.
- Do not create ad-hoc top-level class names that break the block namespace.

**Canonical page skeleton:**

```html
<section class="feature-page-c">
  <div class="feature-page-c__wrapper">
    <div class="feature-page-c__container">
      <div class="feature-page-c__shell">
        <section class="feature-page-c__hero-panel">
          ...
        </section>

        <section class="feature-page-c__content-panel">
          ...
        </section>
      </div>
    </div>
...
**SCSS structure rule:**
- SCSS must be rooted in the same single block selector as the HTML root.
- Write nested SCSS under that root block so the structure mirrors the HTML hierarchy.
- Nest for component structure and state only; do not create deep descendant chains that depend on raw HTML tags.
- Keep selectors low-specificity and block-scoped.
- Media queries should stay inside the component block whenever possible so responsive behavior remains local to the component.

**Canonical SCSS skeleton:**
...
**Required outcomes:**
- HTML and SCSS must read as two views of the same component tree.
- New page work must match this wrapper/container/shell discipline unless a documented exception exists.
- When refactoring old components, move them toward this pattern instead of introducing a second structural style.
...
> **PROMPT KURALI:** Bana herhangi bir SCSS kodu önermeden önce mutlaka bu dokümandaki Design Token tablosunu kontrol et ve sadece buradaki değişkenleri (ar(--...)) kullan.
